...
namespace App\Http\Controllers\CartoesCredito;

use App\Data\BancosSugeridos;
use App\Enum\BandeiraCartaoCredito;
use App\Http\Controllers\Controller;
use App\Http\Requests\CartoesCredito\ArquivarCartaoCreditoRequest;
use App\Http\Requests\CartoesCredito\AtualizarCartaoCreditoRequest;
use App\Http\Requests\CartoesCredito\CriarCartaoCreditoRequest;
use App\Http\Resources\CartoesCredito\CartaoCreditoResource;
use App\Models\CartoesCredito\CartaoCredito;
use App\Services\CartoesCredito\CartaoCreditoService;
use Exception;
...
class CartaoCreditoController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        return Inertia::render('CartoesCredito/Index', [
    ...
    public function arquivarCartaoCredito(ArquivarCartaoCreditoRequest $request, CartaoCredito $cartaoCredito): RedirectResponse
    {
        try {
            ...
            $this->cartaoCreditoService->arquivar(
                $cartaoCredito,
                (int) Auth::id(),
                $request->boolean('arquivada')
            );
            DB::commit();

            $mensagem = $request->boolean('arquivada')
