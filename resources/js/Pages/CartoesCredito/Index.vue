...
import FormularioModal from '@/Pages/CartoesCredito/FormularioModal.vue';
...
const cartoesAtivos = computed(() => props.cartoesCredito.filter((cartao) => !cartao.arquivada));
const cartoesArquivados = computed(() => props.cartoesCredito.filter((cartao) => cartao.arquivada));
...
function alternarArquivada(cartao) {
    router.patch(cartao.url_arquivar, {
        arquivada: cartao.arquivada ? 0 : 1,
    }, {
        preserveScroll: true,
    });
}
                ...
                <div class="divide-y divide-gray-100">
                    ...
                    >
                        ...
                        <div class="flex items-center gap-1 shrink-0">
                            ...
                            <button type="button"
                                class="rounded-lg p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-800"
                                :title="cartao.arquivada ? 'Desarquivar' : 'Arquivar'"
                                @click="alternarArquivada(cartao)"
                            >
                                <span class="material-symbols-outlined text-[20px]">
                                    {{ cartao.arquivada ? 'unarchive' : 'archive' }}
                                </span>
                            </button>
